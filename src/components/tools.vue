<template>
  <div>
    ...
    <div class="tool-box">
      ...
      <span @click="drawingLineModeSwitch('pencil')" :class="!state.isDrawingLineMode && state.isPencil && 'bg'">
        <svg t="1704552845839" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
          p-id="2503" width="20" height="20">
          <path
            d="M581.50172 186.48l256.04 256.04-555.98 555.98-228.28 25.2C22.72172 1027.08-3.09828 1001.24 0.30172 970.68l25.4-228.44 555.8-555.76z m414.4-38.12l-120.22-120.22c-37.5-37.5-98.32-37.5-135.82 0l-113.1 113.1 256.04 256.04 113.1-113.1c37.5-37.52 37.5-98.32 0-135.82z"
            p-id="2504"></path>
        </svg>
      </span>
...
const drawingLineModeSwitch = (isArrow) => {
  if (isArrow === 'pencil') {
    if (state.isDrawingLineMode) {
      state.isDrawingLineMode = false
      return
    }
    state.isPencil = !state.isPencil
    ...
    canvasEditor.canvas.freeDrawingBrush.width = 4;
    ...
    canvasEditor.canvas.forEachObject((obj) => {
      ...
      if (obj.id !== 'workspace') {
        ...
        obj.id = 10000000 * Math.redom + '';
        obj.fill = 'rgb(0,0,0)';
      }
    });

  } else {
    state.isPencil = false
    canvasEditor.canvas.isDrawingMode = false;
    state.isArrow = isArrow;
    state.isDrawingLineMode = !state.isDrawingLineMode;
    canvasEditor.setMode(state.isDrawingLineMode); // 画线功能
    canvasEditor.setArrow(isArrow); // 画哪种线
